...
class OpenAIProvider(LLMProvider):
    ...
    async def complete(self, system: str, user: str, *, temperature: float = 0.2) -> ModelReply:
        data=await self._responses({"model":self.model,"instructions":system,"input":user})
        return ModelReply(text=self._text(data),provider=self.name,model=self.model)

    async def web_search(self, query: str) -> dict:
        data=await self._responses({"model":self.model,"instructions":"Search the public web. Return a concise evidence summary and cite source URLs when available. Do not invent sources.","input":query,"tools":[{"type":"web_search"}]})
        citations=[]
        for item in data.get("output",[]):
            for c in item.get("content",[]):
                for ann in c.get("annotations",[]) or []:
                    ...
                    if url and not any(x["url"]==url for x in citations): citations.append({"title":title or url,"url":url})
        return {"provider":"openai","summary":self._text(data),"citations":citations,"response_id":data.get("id")}
