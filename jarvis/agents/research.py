...
class ResearchAgent(Agent):
    ...
    async def run(self, task: PlanTask, context: str, dependency_results: list[AgentResult]):
        ...
        prompt=f"Research request: {task.instruction}\n\nMemory/context:\n{context}\n\nSearch evidence:\n{evidence or 'No live search provider configured.'}\n\nProduce a useful synthesis. Clearly distinguish evidence from inference."
        reply=await provider.complete("You are JARVIS Research Agent. Never fabricate sources. If live search is unavailable, state that limitation.",prompt)
        return AgentResult(ok=True,summary=reply.text,sources=results,data={"search_provider":search.get("result",{}).get("provider")}),reply.provider,reply.model
