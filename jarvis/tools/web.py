...
def _assert_safe_url(url:str):
    parsed=urlparse(url)
    if parsed.scheme not in {"http","https"} or not parsed.hostname: raise ValueError("Only http/https URLs are allowed")
    host=parsed.hostname.lower()
    if host in {"localhost","127.0.0.1","::1"} or host.endswith(".local"): raise ValueError("Local/private hosts are blocked")
    try:
        for info in socket.getaddrinfo(host,None):
            ip=ipaddress.ip_address(info[4][0])
            if ip.is_private or ip.is_loopback or ip.is_link_local or ip.is_reserved: raise ValueError("Private network targets are blocked")
...
class WebFetchTool(Tool):
    ...
    async def run(self,url:str):
        ...
        async with httpx.AsyncClient(timeout=20,follow_redirects=True,headers={"User-Agent":"JARVIS/0.3"}) as client:
            r=await client.get(url); r.raise_for_status()
        ctype=r.headers.get("content-type","")
        if "text" not in ctype and "json" not in ctype: raise ValueError("Only text/json resources are supported")
        return {"url":str(r.url),"status":r.status_code,"text":r.text[:30000]}

class WebSearchTool(Tool):
    name="web.search"; description="Search the public web. Uses Tavily/Serper when configured, otherwise OpenAI hosted web search."; risk=RiskLevel.LOW
    async def run(self,query:str,max_results:int=5):
        ...
        if openai.available:
            result=await openai.web_search(query); citations=result.get("citations",[])[:max_results]
            rows=[{"title":x.get("title"),"url":x.get("url"),"snippet":result.get("summary","")} for x in citations]
            if not rows: rows=[{"title":"OpenAI web search synthesis","url":None,"snippet":result.get("summary","")}]
            return {"provider":"openai","results":rows,"summary":result.get("summary","")}
        return {"provider":None,"results":[],"warning":"Primary OpenAI API is not configured."}
